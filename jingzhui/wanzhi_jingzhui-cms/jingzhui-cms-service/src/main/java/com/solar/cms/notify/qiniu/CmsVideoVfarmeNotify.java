...
@Slf4j
@Component
public class CmsVideoVfarmeNotify implements QiniuNotify {
	
	public static String TASK_TYPE = "cms_video_vframe";
	...
	public void notify(String taskId, String fileKey, String content, boolean success) {
		log.info("视频缩略图处理通知,taskId={},文件ID={},success={},", taskId, fileKey, success);
		if (!success) {
			log.info("视频缩略图处理失败,文件ID={}", fileKey);
			return ;
		}
		...
		if (video != null && StringUtils.equalsIgnoreCase(video.getVsampleTaskId(), taskId)) {
			video.setVsampleStatus("Y");
